 ...
 */
 ...
 function wordsList (str, subStr) {
  ...
  for (let i = 0; i < n.length; i++) {
    slovo = n[i]
    var reg = slovo.toLowerCase()
    if (reg.includes(subStr)) {
      sim = reg.replace('.', '').replace(',','').replace('!','').replace('?','').replace(';','').replace('\"','')
      result.add(sim)
    }
  }
  return result;
 ...
 */
 ...
 var car2 = new Car(5000, 'FX50 AWD', 'Infiniti', 2012);
 ...
 */
 ...
 var testPerformance = (interations, func) => {
   var time = Date.now();
